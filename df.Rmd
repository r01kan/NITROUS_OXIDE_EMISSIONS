...
Some data sets still didn't match the entities of nitrous oxide emissions.
The code below matches every data set to the nitroux oxide emissions data set, each data set now has 201 observations.

```{r}
entities_df <- data.frame(Entity = unique(n_em_entities$Entity))
adjust_dataset <- function(df, entities_df) {
+     # Perform a left join
+     adjusted_df <- left_join(entities_df, df, by = "Entity")
+     return(adjusted_df)
+ }
> 
> adjusted_agr_land <- adjust_dataset(agr_land, entities_df)
> adjusted_ann_growth_exp <- adjust_dataset(ann_growth_exp, entities_df)
> adjusted_ann_growth_imp <- adjust_dataset(ann_growth_imp, entities_df)
> adjusted_forest_area <- adjust_dataset(forest_area, entities_df)
> adjusted_gdp_per_cap <- adjust_dataset(gdp_per_cap, entities_df)
> adjusted_meat_supp_per_cap <- adjust_dataset(meat_supp_per_cap, entities_df)
> adjusted_n_em <- adjust_dataset(n_em, entities_df)
> adjusted_n_em_per_cap <- adjust_dataset(n_em_per_cap, entities_df)
> adjusted_n_em_per_sec <- adjust_dataset(n_em_per_sec, entities_df)
> adjusted_pop_den <- adjust_dataset(pop_den, entities_df)
> adjusted_urban_v_rural <- adjust_dataset(urban_v_rural, entities_df)
> adjusted_water_resc <- adjust_dataset(water_resc, entities_df)
> adjusted_wetlands <- adjust_dataset(wetlands, entities_df)
```{r}

Changing names of datasets

```{r}
total_agricultural_land <- adjusted_agr_land
annual_exports_growth <- adjusted_ann_growth_exp
annual_imports_growth <- adjusted_ann_growth_imp
total_forest_area <- adjusted_forest_area
gdp_per_capita <- adjusted_gdp_per_cap
meat_supply_per_capita <- adjusted_meat_supp_per_cap
nitrous_oxide_emissions_CO2equivilent <- adjusted_n_em
nitrous_oxide_emissions_CO2eq <- nitrous_oxide_emissions_CO2equivilent
rm(nitrous_oxide_emissions_CO2equivilent)
per_capita_nitrous_oxide_emissions_CO2eq <- adjusted_n_em_per_cap
per_capita_nitrous_oxide_emissions_CO2eq_per_sector <- adjusted_n_em_per_sec
population_density <- adjusted_pop_den
urban_vs_rural <- adjusted_urban_v_rural
renewable_freshwater <- adjusted_water_resc
wetland_area_shares <- adjusted_wetlands
```{r}
